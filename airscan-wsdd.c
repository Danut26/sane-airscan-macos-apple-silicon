...
wsdd_finding_new (int ifindex, const char *address)
{
    ...
    wsdd->finding.uuid = airscan_uuid_parse(address);
...
wsdd_sock_enable_pktinfo_ip6 (int fd)
{
    ...
    rc = setsockopt(fd, IPPROTO_IPV6, IPV6_2292PKTINFO, &yes, sizeof(yes));
    if (rc < 0) {
        log_debug(wsdd_log, "setsockopt(AF_INET6, IPV6_2292PKTINFO): %s",
                strerror(errno));
    }
...
wsdd_resolver_new (const netif_addr *addr, bool initscan)
{
    ...
    resolver->fd = socket(af, SOCK_DGRAM, 0);
    if (resolver->fd < 0) {
